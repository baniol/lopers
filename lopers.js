...
//     https://github.com/baniol/lopers
var Lopers = function(dbName, options) {
	...
	this.insert = function(table, arr, fn) {
		...
		for (var j in this._db) {
			if (this._db[j].table == table) {
				this._db[j].records.push(el);
			}
		}
	...
	this._getFirstCid = function() {
		var newCid = parseInt(this._lastCid,10);
		newCid++;
